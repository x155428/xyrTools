...
<template>
  <div class="ledger-container">
    ...
    <div class="ledger-content">
      ...
      <div class="ledger-data">
        <div v-if="selectedNode && !selectedNode.isFolder&&gridOptions.schema.data.data.length>0" class="data-header">
          <h3>{{ selectedNode.name }}</h3>
          <div class="data-actions">
            <el-button type="primary" @click="handleAddRecord" size="small" :disabled="!isEdit">新增记录</el-button>
            <el-button type="primary" @click="handleEditLedger(gridOptions.schema.data.ledgerId)" size="small"
        ...
        <div class="account-content">
          <template v-if="selectedNode && !selectedNode.isFolder">
            <vxe-grid ref="gridRef"
              v-bind="{...gridOptions.gridConfig,columns:gridOptions.schema.columns,data:gridOptions.schema.data.data || []}"
              :height="'100%'" @context-menu="handleContextMenu" @cell-click="onCellClick"
              :edit-config="isEdit ? { trigger: 'click', mode: 'row' } : null">
  ...
  <!-- 行右键菜单 -->
  <div v-show="rowContextMenuVisible"
    :style="{left: rowContextMenuPosition.x + 'px', top: rowContextMenuPosition.y + 'px'}" class="custom-context-menu">
    <div class="context-menu-item" @click="handleInsertRowBefore">
      在前面插入
    </div>
    <div class="context-menu-item" @click="handleInsertRowAfter">
      在后面插入
    </div>
    <div class="context-menu-item" @click="handleDeleteRow(selectedRow)">
      删除当前行
    </div>
  </div>
...
<script setup>
  ...
  const rowContextMenuVisible = ref(false) // 行右键菜单是否可见
  ...
  const handleNodeClick = async (data) => {
    ...
    // 加载台账数据
    gridOptions.gridConfig.loading = true
    try {
      ...
      if (tableCfgRes) {
        Object.assign(gridOptions.gridConfig, tableCfgRes)
      }
      gridOptions.schema.columns = columnRes.columns || []
      gridOptions.schema.data = dataRes || []
      console.log(gridOptions.schema.data.data)
    } finally {
      gridOptions.gridConfig.loading = false
    }
    // 重新渲染表格
    if (gridRef.value) {
  ...
  const loadLedgerData = async (ledger) => {
    ...
    try {
      ...
      // 更新表格配置
      gridOptions.schema.columns = finalColumns
      gridOptions.schema.data = dataRes.data || []
    } catch (error) {
  ...
  // 处理台账右键菜单事件
  const handleContextMenu = ({ $event, row, rowIndex }) => {
    // 非编辑模式禁止右键操作
    if (!isEdit.value) {
      return
    }
    ...
    // 显示行右键菜单
    rowContextMenuPosition.x = event.clientX
    rowContextMenuPosition.y = event.clientY
  ...
  const handleInsertRowBefore = () => {
    ...
    // 为每个列添加默认值
    gridOptions.schema.columns.forEach(column => {
      // 跳过序号列和操作列
    ...
    gridOptions.schema.data.splice(selectedRowIndex.value, 0, newRecord)
  ...
  const handleInsertRowAfter = () => {
    ...
    gridOptions.schema.data.splice(selectedRowIndex.value + 1, 0, newRecord)
  ...
  const handleEditLedger = (ledgerId) => {
    if (!gridOptions.schema.data || gridOptions.schema.data.data.length === 0 || selectedNode.value.type !== 'ledger') {
      ElMessage({
        message: '请先选择一个台账',
        type: 'warning',
        grouping: true,
    ...
    columnsConfig.value = JSON.parse(JSON.stringify(gridOptions.schema.columns))
  ...
  const confirmCreateLedger = () => {
    ...
    if (isEditMode.value && gridOptions.schema.data.ledgerId) {
      // 有id 编辑模式：更新现有台账
  ...
  const handleAddRecord = () => {
    ...
    gridOptions.schema.data.push(newRecord)
  ...
  const handleDeleteRow = (row) => {
    ElMessageBox.confirm(
      ...
      const index = gridOptions.schema.data.findIndex(item => item.id === row.id)
      if (index > -1) {
        gridOptions.schema.data.splice(index, 1)
      }
    }).catch(() => {
      // 取消操作
    })
